...
const CodeSwitcher: React.FC = React.memo(() => {
  const [selectedType, setSelectedType] = useState(TYPES[0]);
  const { user } = useUser();
  const { session } = useSession();
  const { organization } = useOrganization();

  const selectedCode = useMemo(() => JSON.stringify(
    {
      user,
      session,
      organization,
  ...
  const typesToShow = useMemo(() => (
    organization ? TYPES : TYPES.filter(type => type !== "organization")
  ), [organization]);
    ...
    <div className={clsx(organization ? "h-[54.625rem]" : "h-[41.625rem]")}>
      ...
      <div className="relative h-[calc(100%-42px)]">
        <div className="mask h-full">
          {/* @ts-expect-error */}
          <SyntaxHighlighter language="javascript" style={theme as PrismTheme}>
            {selectedCode}
          </SyntaxHighlighter>
        </div>
